...
class IdbRepository extends MemoryRepository {
  ...
  Future<void> init() async {
    ...
    _loading = true;
    await _loadRoot(root);
    _loading = false;
  }

  Future<void> _loadRoot(Map<String, dynamic> root) async {
    await wipe();
  ...
  Future<void> persist() async {
    ...
    await _write(await _snapshotRoot());
  }

  Future<void> _write(Map<String, Object?> root) async {
    final txn = _db!.transaction(storeName, 'readwrite');
    await txn.objectStore(storeName).put(jsonEncode(root), rootKey);
    await txn.completed;
  }

  Future<Map<String, Object?>> _snapshotRoot() async {
